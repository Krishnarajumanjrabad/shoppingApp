...
var OrderDetailPage = /** @class */ (function () {
    function OrderDetailPage(navCtrl, navParams, db, storage, popoverCtrt, auth, utility, orderConfirm, productService) {
        this.navCtrl = navCtrl;
        this.navParams = navParams;
        this.db = db;
        this.storage = storage;
        this.popoverCtrt = popoverCtrt;
        this.auth = auth;
        ...
        if (this.productList.length > 0) {
            ...
            for (var _i = 0, _a = this.productList; _i < _a.length; _i++) {
                ...
                if (product.productSpecficationInfo.length > 0) {
                    for (var _b = 0, _c = product.productSpecficationInfo; _b < _c.length; _b++) {
                        var specification = _c[_b];
    ...
    OrderDetailPage.prototype.productRemove = function (product, i) {
    ...
    OrderDetailPage.prototype.getGrandTotal = function () {
    ...
    OrderDetailPage.prototype.onSelectCountry = function (countryId) {
        var _this = this;
        if (countryId) {
            this.countryListNew = this.db.list("/countries");
            this.countryListNew.subscribe(function (items) {
    ...
    OrderDetailPage.prototype.onStateSelection = function (stateName) {
    ...
    OrderDetailPage.prototype.saveOrderInfo = function (orderForm, productList) {
        var _this = this;
        var orderId;
        if (orderForm && productList) {
            return new Promise(function (resolve, reject) {
                _this.orderInfoList = _this.orderConfirm.getOrderDetais();
                _this.orderInfoList.subscribe(function (results) {
                    if (results.length > 0) {
                ...
                _this.productService.updateProductInventory(productList);
                _this.orderConfirm.saveOrderDetails(orderForm, productList, _this.grandTotalPrice, res, _this.userInfo).then(function (result) {
                    console.log("inside the order save detauls result");
                    console.log(result);
                    _this.navCtrl.push(OrderConfirmationPage, { "orderInfo": result });
                });
            }).catch(function (err) {
                console.log(err);
                if (_this.firstTime) {
                    _this.productService.updateProductInventory(productList);
                    _this.orderConfirm.saveOrderDetails(orderForm, productList, _this.grandTotalPrice, err, _this.userInfo).then(function (res) {
                        _this.navCtrl.push(OrderConfirmationPage, { "orderInfo": res });
                    });
                }
            });
        }
    ...
    OrderDetailPage.prototype.showLogin = function () {
        var _this = this;
        var popover = this.popoverCtrt.create(LoginPage);
        popover.present();
        //  console.log( this.navParams.get('user') );
        //   console.log(this.user);
        popover.onDidDismiss(function (popoverData) {
            _this.userInfo = popoverData;
            console.log("getting the userinformation");
            console.log(_this.userInfo);
        });
    };
    OrderDetailPage.prototype.logOff = function () {
    ...
    OrderDetailPage.prototype.activateGuestCheckout = function () {
        this.userInfo = new UserInformation("", "", "", "", 0, "", "", 0, "", "");
    };
    OrderDetailPage = __decorate([
        ...
        __metadata("design:paramtypes", [NavController, NavParams, AngularFireOfflineDatabase, NativeStorage, PopoverController, AuthenicateServiceProvider, UtilityService, OrderconfirmationserviceProvider, ProductServiceProvider])
    ], OrderDetailPage);
